...
requireText(contentPacks, "findCurrentCachedAssetResponse", "runtime cache contract");
...
requireText(contentPacks, "type.startsWith(\"audio/\")", "audio response type validation");
...
requireText(durableProfile, "request.result.close()", "late IndexedDB connection cleanup");
...
requireText(engine, "persistProgress", "preview persistence isolation");
requireText(engine, "gameCleanupState", "game cleanup diagnostics");
requireText(parent, "previewMode: true", "adult preview contract");
...
requireText(session, "previewMode", "preview session branch");
...
console.log(
  "Stability hardening valid: current-release offline repair/playback, bounded audio/IndexedDB, preview isolation, generation-safe emergency persistence, cleanup isolation and bootstrap recovery are guarded.",
);
